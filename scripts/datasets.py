...
class PVDataset(Dataset):
    ...
    def __init__(self, features: np.ndarray, targets: np.ndarray, seq_len: int, scalerX: StandardScaler = None,
                 scalerY: StandardScaler = None, shuffle: bool = False):
...
class DatasetBuilder:
    ...
    def build_dataset(self):
        ...
        return PVDataset(self.X.values, self.Y.values,  seq_len=self.seq_len)
    ...
    def split_dataset(self, train_ratio: float, val_ratio: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        test_split = int((train_ratio + val_ratio + test_ratio) * len(self.Y))
        self.train_idx, self.val_idx, self.test_idx = indices[:train_split], indices[train_split:val_split], indices[
                                                                                                            val_split:]
        return self.train_idx, self.val_idx, self.test_idx

    def build_split_datasets(self, train_idx: int, val_idx: int, test_idx: int) -> tuple[PVDataset, PVDataset, PVDataset]:
        '''
        :param train_idx: int
        :param val_idx: int
        :param test_idx: int
        ...
        return (PVDataset(self.X_train, self.Y_train, seq_len=self.seq_len, scalerX=scalerX, scalerY=scalerY, shuffle=True),
                PVDataset(self.X_validation, self.Y_validation, seq_len=self.seq_len),
                PVDataset(self.X_test, self.Y_test, seq_len=self.seq_len))
    ...
    def preprocess_pv_history(self):
        ...
        self.pv_history = self.pv_history_raw[self.pv_history_raw['pv_generation'] >= 0]
        ...
        self.pv_history = self.pv_history[(self.pv_history["pv_generation"] >= Q1 - 1.5 * IQR) &
                         (self.pv_history["pv_generation"] <= Q3 + 1.5 * IQR)]
...
if __name__ == "__main__":
    ...
    dataset, _, _, _, _, _, _ = dataset_builder.build_dataset(train_split=0.6, val_split=0.2)
